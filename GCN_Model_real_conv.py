...
test_sentence = model_embedding.encode(["This is test sentence"])
size_embedding = len(test_sentence[0])
...
for transcription_id in tqdm.tqdm(training_transcription_ids, desc="Processing transcriptions"):
    ...
    with open(path_to_training / f"{transcription_id}.json", "r") as file:
        ...
        #Create graph
        # Complete the graph with 0s
        if len(X) < max_graph_size: 
            embedding_text = np.concatenate((embedding_text, np.zeros((max_graph_size - len(X), size_embedding))))
            edge_attr = np.concatenate((edge_attr, np.zeros((max_graph_size - len(X), size_dict))))
            labels = np.concatenate((labels, np.zeros(max_graph_size - len(X))))
...
class Model(torch.nn.Module):
    def __init__(self, size_input=size_input, num_features=num_features, hidden_size=size_hidden, num_classes=size_output):
...
input_size = size_embedding
intermediate_size = size_embedding
dropout = 0.2
final_size = 1
threshold = 0.25
